...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  const colorConfig = Object.entries(config).filter(([, c]) => (c as any).theme || (c as any).color);
          ...
          return (
            <div key={item.dataKey} className={cn("flex w-full flex-wrap items-stretch gap-2", indicator === "dot" && "items-center")}>
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  <div className={cn("shrink-0 rounded-[2px] border-border bg-(--color-bg)", indicator === "dot" ? "h-2.5 w-2.5" : "w-1")} style={{"--color-bg": indicatorColor} as any} />
